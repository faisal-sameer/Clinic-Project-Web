...
class ReceptionController extends Controller
{
    ...
    protected function AppointmentsAccepted(Request $request)
    {
        ...
        $test->sendGCM("تم تاكيد موعدك", $patient->Date, $patient->Token, "1", "w");
        // }
    ...
    protected function AppointmentsRejected(Request $request)
    {
        ...
        if ($patient->Token != null) {
            ...
            $test->sendGCM("تم رفض موعدك", $patient->Date, $patient->Token, "1", "w");
        }
    ...
    protected function PatientCompleted(Request $request)
    {
        Reservation::where('id', $request->id)->update([
            'Status' => 5
        ]);
    ...
    protected function PatientDidNotCome(Request $request)
    {
        Reservation::where('id', $request->id)->update([
            'Status' => 6
        ]);
